...
import { Dispatch, FC, RefObject, useContext, useEffect, useRef, useState } from "react";
    ...
    <>
      ...
      {Dropdown && (
        <ListDropdowStyled ref={wrapperRef}>
          {carshow.length === 0 ? (
            <AtomWrapper padding="30px 0px" backgroudColor="#E2E0E0" justifyContent="center" alignItems="center">
              <AtomTextBody>No hay productos en el carrito</AtomTextBody>
            </AtomWrapper>
          ) : (
            carshow.map((e) => (
              <MoleculeItemList size={e.size} quanti={e.quanti} id={e.id} image={e.image} preci={e.preci} name={e.name} />
            ))
          )}
        </ListDropdowStyled>
      )}
